<template>
  <div class="my-profile">
    ...
    <div class="go__messages" @click="toggleMessageVisibility">
      ...
      <div
        v-if="areProfileNotifications && !messagesVisible"
      ...
      <div class="notifications-container" v-if="messagesVisible">
        ...
        <transition-group tag="ul" name="fade" class="messages-list">
          <MessageItem
            ...
            :data="el"
            @confirm="
              acceptFriendship(index, el.friendID, el.username, el.messagePic)
            "
          ...
          />
          <div v-if="requests.length == 0 && arenas.length == 0">
            No more messages
          </div>
        </transition-group>
      </div>
...
export default {
  ...
  methods: {
    ...
    acceptFriendship(index, id, name, img) {
      ...
      user.collection("friend-requests").doc(name).delete();
